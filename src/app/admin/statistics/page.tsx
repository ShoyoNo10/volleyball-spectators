...
export default function StatisticAdmin({
  title = "🏐 Admin • Оноо",
  apiPath = "/api/statistics",
}: {
  title?: string;
  apiPath?: string; // default "/api/statistics"
}) {
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      // ✅ статистик ч гэсэн gender query-тэй
      const res = await fetch(`${apiPath}?gender=${gender}`);
      const data: unknown = await res.json();
      setRows(Array.isArray(data) ? data.filter(isStatRow) : []);
    } finally {
      setLoading(false);
    }
  };
  ...
  // ✅ Upload → /api/upload/logo (чи яг ийм route-той гэж хэлсэн)
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      if (typeof url === "string" && url.length > 0) {
        ...
        alert("Upload failed: url/secure_url ирсэнгүй (Network->Preview шалга)");
      }
    } finally {
      setUploading(false);
    }
  ...
  const onSubmit = async () => {
    if (!playerName || !teamCode)
      return alert("playerName/teamCode хоосон байна");
    ...
    if (editingId) {
      // ✅ чи GenericStatAdmin дээрээ PUT ашигласан тул адилхан байлгалаа
